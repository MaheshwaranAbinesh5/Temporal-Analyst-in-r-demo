# The Temporal Analyst - M.Abinesh(34614)
## Analysis of Long-Term Air Quality Trends (2010–2022)
...
region_data <- df_filtered %>%
  filter(Country %in% c("India", "Sri Lanka", "Pakistan", "Bangladesh"))
```
...
ggplot(missing_data, aes(x = Year)) +
  geom_bar(aes(y = missing_PM25, fill = "PM2.5"), stat = "identity") +
  geom_bar(aes(y = missing_PM10, fill = "PM10"), stat = "identity", alpha = 0.7) +
  geom_bar(aes(y = missing_NO2, fill = "NO2"),  stat = "identity", alpha = 0.5) +
...
long_data <- yearly_summary %>%
  pivot_longer(cols = starts_with("mean_"),
               names_to = "Pollutant",
               values_to = "Value")

ggplot(long_data, aes(x = Year, y = Value, color = Pollutant, group = Pollutant)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(title = "Air Quality Trends (2010–2022) - South Asia",
...
The analysis of air pollutants (PM2.5, PM10, and NO₂) between 2010 and 2022 in the South Asian region reveals a gradual improvement in air quality.  
PM2.5 and PM10 concentrations show a steady decline after 2016, while NO₂ remains relatively stable.  
This indicates that industrial and vehicular emissions continue to impact air quality but are offset by environmental regulations and cleaner energy policies.  
Data availability is uneven across years (e.g., 2011–2013), which may slightly affect accuracy.


### Conclusion
Based on aggregated yearly trends, the selected region shows an overall improvement in air quality from 2010 to 2022.  
Particulate matter (PM2.5 and PM10) levels have declined, suggesting positive effects of policy interventions,  
while NO₂ levels remain nearly constant.  
Overall, this supports the argument that air quality in South Asia is improving, though further emission reductions are still needed.
